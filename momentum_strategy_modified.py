...
class Strategy(bt.Strategy):
    def __init__(self):
        ...
        self.quarterly_sentiments = quarterly_sentiments
    ...
    def rebalance_portfolio(self):
        ...
        # Fetch sentiment data for the last quarter
        current_date = self.datas[0].datetime.date(0)
        current_quarter = pd.to_datetime(current_date).to_period("Q")
        quarter = current_quarter - 1
        
        composite_momentum = {}
        if quarter in self.quarterly_sentiments.index:
            sentiments = self.quarterly_sentiments.loc[quarter]
            for d in self.rankings:
                ticker = d._name
                if ticker in sentiments:
                    # Calculate composite momentum
                    composite_momentum[d] = 0.4 * self.inds[d]["momentum"][0] + 0.6 * sentiments[ticker]
                else:
                    composite_momentum[d] = self.inds[d]["momentum"][0]

            # Sort stocks based on the composite momentum
            self.rankings.sort(key=lambda d: composite_momentum[d], reverse=True) 
        else:
